...
Ovo je Aplikacija za provjeru listi, ideja je da kada se razvije određeni dio ove aplikacije da radi ogromne liste provjera koje bi olakšale posao ljudima i dale bi detaljan feedback na temelju onoga što zasigurno nije u redu sa poslovanjem zasada će provjeravati samo potencijalne točke koji nisu pravilno formirani kao razriješeni ciljevi.

## Opis programskog koda

U početnom dijelu se uvode sve relevantne biblioteke za rad sa podatcima sa navedenim programskim kodom:

```py
  import sys, numpy as np, pandas as pd, matplotlib.pyplot as plt
```

Nakon uvođnja biblioteka se kreiraju varijable i varijabilna polja koja se koriste kroz cijeli programski kod:
...
Poslije kreiranja varijabla i varijabilnog polja se kreiraju funkcije koje formiraju osnovne funkcionalnosti programa:
...
Poslije formiranja osnovnih funkcionalnosti sve funkcionalnosti se uvedu unutar novo formirane glavne funkcije:
...
U finalnom dijelu programskog koda se pozove funkcija main:
...
Kroz detaljne funkcionalnosti ideja je da svaka funkcionalnost riješi određeni dio problema kako bi se pružalo riješenje za poslovnu analizu pojedinog poslovanja bez obzira o kakvoj djelatnosti se radi, program se sastoji od više algoritama koji zajedno formiraju jedan algoritam za analizu rada.

## Ideja algoritma

Ideja algoritma je da pruža funkcionalnosti izbornika, napomene, unosa, provjere te tablične i grafičke vizualizacije što bi omogučilo detaljne funkcionalnosti u obliku python funkcija koje se povežu u jednu main funkciju i tako se pokrene cjelokupni rad programskog koda.
